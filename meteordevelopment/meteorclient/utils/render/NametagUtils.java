...
public class NametagUtils {
    ...
    public static void onRender(Matrix4f modelView) {
        ...
        Utils.set(camera, mc.field_1773.method_19418().method_19326());
        cameraNegated.set(camera);
        cameraNegated.negate();

        windowScale = mc.method_22683().method_4476(1, false);
    }
    ...
    public static boolean to2D(Vector3d pos, double scale, boolean distanceScaling, boolean allowBehind) {
        ...
        toScreen(pmMat4);
        double x = pmMat4.x * mc.method_22683().method_4489();
        double y = pmMat4.y * mc.method_22683().method_4506();

        if (behind) {
            x = mc.method_22683().method_4489() - x;
            y = mc.method_22683().method_4506() - y;
        }

        if (Double.isInfinite(x) || Double.isInfinite(y)) return false;

        pos.set(x / windowScale, mc.method_22683().method_4506() - y / windowScale, allowBehind ? pmMat4.w : pmMat4.z);
        return true;
    }
    ...
    public static void begin(Vector3d pos, class_332 drawContext) {
        ...
        matrices.method_46416((float) pos.x, (float) pos.y, 0);
        matrices.method_22905((float) scale, (float) scale, 1);
    }
    ...
    private static double getScale(Vector3d pos) {
        ...
        return class_3532.method_15350(1 - dist * 0.01, 0.5, Integer.MAX_VALUE);
    }
