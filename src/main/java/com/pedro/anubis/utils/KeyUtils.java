...
public final class KeyUtils {
    ...
    /** Özel birkaç code'u elle isimlendir, kalanı glfwGetKeyName ile. */
        ...
        return switch (key) {
            ...
            default -> {
                ...
                if (name == null || name.isBlank())
                    yield "None";
                yield capitalizeAscii(name);
            }
        };
    }

    /**
     * Not: Mouse button için GLFW_MOUSE_BUTTON_* 0..7 genelde.
     */
    ...
    /**
     * Bound key'i (mouse/keysym/scancode) üzerinden fiziksel basılı mı kontrol
     * eder.
     */
    public static boolean isPressed(InputUtil.Key key) {
        MinecraftClient mc = mc();
        if (mc == null || mc.getWindow() == null || key == null)
            return false;
        ...
        return switch (key.getCategory()) {
            ...
            case KEYSYM -> GLFW.glfwGetKey(window, key.getCode()) == GLFW.GLFW_PRESS;
            case SCANCODE -> {
                int keyCode = scancodeToKeyCode(key.getCode());
                yield keyCode != -1 && GLFW.glfwGetKey(window, keyCode) == GLFW.GLFW_PRESS;
            }
        };
    }

    private static int scancodeToKeyCode(int scancode) {
        // SCANCODE nadir kullanılır; brute-force yeterli.
    ...
    private static String capitalizeAscii(String s) {
        if (s == null || s.isEmpty())
            return s;
        char first = s.charAt(0);
        char up = (first >= 'a' && first <= 'z') ? (char) (first - 32) : first;
        return (s.length() == 1) ? String.valueOf(up) : up + s.substring(1);
    }
}
