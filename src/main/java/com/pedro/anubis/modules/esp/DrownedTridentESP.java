...
public class DrownedTridentESP extends Module {
    ...
    public DrownedTridentESP() {
        super(AnubisAddon.ESP, "drowned-trident-esp", "Highlights drowned mobs holding tridents.");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (!(entity instanceof DrownedEntity drowned)) continue;
            if (!isHoldingTrident(drowned)) continue;

            Box box = drowned.getBoundingBox();
            event.renderer.box(box, espColor, espColor, shapeMode, 0);

            if (lineColor != null) {
                double x = MathHelper.lerp(event.tickDelta, drowned.prevX, drowned.getX());
                double y = MathHelper.lerp(event.tickDelta, drowned.prevY, drowned.getY());
                double z = MathHelper.lerp(event.tickDelta, drowned.prevZ, drowned.getZ());
